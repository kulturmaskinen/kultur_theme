...
function kultur_theme_preprocess_html(&$vars) {
  ...
  if (!empty($node) && !empty($node->field_image)) {
    $bg1200 = file_create_url($node->field_image[LANGUAGE_NONE][0]['uri']);
    $bg1600 = file_create_url($node->field_image[LANGUAGE_NONE][1]['uri']);
